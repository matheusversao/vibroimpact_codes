...
% This script handles the creation and writing of the .txt file with simulation information.

function fileID = create_simulation_info(filename, simulation_params)
    % Open the file for writing simulation parameters and impact details
    ...
    %fprintf(fileID, 'OMEGA = %.3f rad/s\n', simulation_params.OMEGA);
    %fprintf(fileID, 'F0 = %.3f N\n', simulation_params.F0);
    fprintf(fileID, 'e = %.2f \n', simulation_params.e);

    fprintf(fileID, 'Initial Conditions:\n');
    %fprintf(fileID, 'Xc0 = %.4f m\n', simulation_params.Xc0);
    fprintf(fileID, 'Xp10 = %.4f m\n', simulation_params.Xp10);
    %fprintf(fileID, 'Vc0 = %.4f m/s\n', simulation_params.Vc0);
    ...
    %fprintf(fileID, 'Animation parameters:\n');
    %fprintf(fileID, 'Heigth = %.3f mm\n', simulation_params.Height);
    %fprintf(fileID, 'Width = %.3f mm\n', simulation_params.Width);
    %fprintf(fileID, 'Thickness = %.3f mm\n', simulation_params.th);
    %fprintf(fileID, 'scale_factor = %.3f\n', simulation_params.scale_factor);

    % Write the impact details header
    fprintf(fileID, '\nImpact Details:\n');
